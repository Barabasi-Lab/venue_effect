...
Usage:
    ...
    python matching_art.py --venue venice_biennale --n_jobs 16 --slope_max_diff 0.10
    python matching_art.py --venue venice_biennale --n_jobs 16 --slope_max_diff 0
...
DEFAULT_PARAMS = {
    ...
    # Slope filter for parallel trends enforcement
    'slope_max_diff': 0.15,   # max |treated_slope - control_slope| per year
    'slope_cols': ['S', 'G'],  # check slopes for these outcomes
}
...
def ols_slope(x, y):
    """OLS slope of y on x. Returns float or nan."""
...
def _match_one(args):
    ...
    for aid, gp in pot_pre.groupby('artist_id'):
        dists[aid] = dtw_distance(t_pre, gp, cols,
    ...
    # ------------------------------------------------------------------
    # Slope filter: walk through DTW-ranked candidates, keep only those
    # whose pre-treatment slope in slope_cols is within ±slope_max_diff.
    # This enforces parallel trends by rejecting controls that grow
    # faster or slower than the treated in the pre-treatment window.
    # ------------------------------------------------------------------
    ...
    if slope_max and slope_max > 0 and slope_cols:
        ...
        # Walk through candidates in DTW order
        top_ids = []
...
def run_matching(treated_df, control_df, venue_key, params=None,
    ...
    m_out = m_out.drop(columns=[c for c in drop_cols if c in m_out.columns],
                       errors='ignore')
