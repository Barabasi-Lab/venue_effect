...
export delimited "..\..\..\results\science\nature_exposure_cit_atet.csv", replace
...
* Save ATET results using parmest
parmest, saving(nature_exposure_prod_atet, replace)
use nature_exposure_prod_atet.dta, clear
gen cohort_number = real(regexs(1)) if regexm(parm, "([0-9]+)\.year_to_publish")
gen to_year = cohort_number - 5
drop parm cohort_number eq

* Save the modified dataset as a CSV file
export delimited "..\..\..\results\science\nature_exposure_prod_atet.csv", replace
...
* Save ATET results using parmest
parmest, saving(nature_exposure_grant_atet, replace)
use nature_exposure_grant_atet.dta, clear
gen cohort_number = real(regexs(1)) if regexm(parm, "([0-9]+)\.year_to_publish")
gen to_year = cohort_number - 5
drop parm cohort_number eq

* Save the modified dataset as a CSV file
export delimited "..\..\..\results\science\nature_exposure_grant_atet.csv", replace